...
export async function readPassphrase({ confirm = false } = {}) {
  // Mimix fork modification: allow env-var passphrase for non-interactive
  // automation. The agent runtime cannot present a TTY prompt to the operator,
  // and persona wallets are ephemeral (created+destroyed per run), so the
  // safety concern around persisted credentials does not apply.
  if (process.env.MIMIX_PASSPHRASE) {
    return process.env.MIMIX_PASSPHRASE;
  }
